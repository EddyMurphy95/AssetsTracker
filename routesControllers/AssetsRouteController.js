...
const { Auth, isAdmin,isUser } = require("../middleware/verifyToken");
...
route.get("/assets", Auth, isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (check.length === 0) {
      resp["status"] = "06";
      resp["message"] =
        "Your asset collection is empty. Kindly create an asset";
...
route.get("/users", Auth, isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (check.length === 0) {
      resp["status"] = "06";
      resp["message"] =
        "Your user collection is empty. Kindly signup an account";
      return res.status(404).send(resp);
    }
...
route.get("/view-assigned-assets/:userId", Auth, isUser, async (req, res) => {
  resp = {};
  try {
...
route.post("/requestAsset", Auth, async (req, res) => {
  ...
  try {
    ...
    if (assigningAsset) {
      const updatedUser = await UserAssetUpdate(
        req.user._id,
        assigningAsset._id
      );
      resp["status"] = "01";
...
route.post("/assetIssue", Auth,isUser,async (req, res) => {
  ...
  if (!req.body.userId || !req.body.assetId || !req.body.message) {
    ...
    }  ${!req.body.message && "message"} is not available`;
  ...
  await Assets.findOneAndUpdate(
    ...
    { $set: { issues: req.body.message } }
  );
  await User.findByIdAndUpdate(
    { _id: req.body.userId },
    { $set: { messages: req.body.message } }
  );
